...
//Read commands.
	...
	//SHA-1
		else if (Command == HASH_COMMAND_SHA1_UNDERLINE || Command.find(HASH_COMMAND_SHA1) == 0)
		{
			HashFamilyID = HASH_ID_SHA1;
		}
	//SHA-2 family
		else if (Command == HASH_COMMAND_SHA2_384 || Command.find(HASH_COMMAND_SHA2_512) == 0 || 
			Command == HASH_COMMAND_SHA2_UNDERLINE || Command.find(HASH_COMMAND_SHA2) == 0)
		{
			HashFamilyID = HASH_ID_SHA2;
			if (!ReadCommands_SHA2(Command))
				return EXIT_FAILURE;
		}
	//SHA-3 family
		else if (Command == HASH_COMMAND_SHA || Command == HASH_COMMAND_SHA3_UNDERLINE || 
			Command.find(HASH_COMMAND_SHA3) == 0)
		{
			HashFamilyID = HASH_ID_SHA3;
			if (!ReadCommands_SHA3(Command))
				return EXIT_FAILURE;
		}
	//Commands error
		else {

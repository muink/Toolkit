...
//Read parameter
	...
	//Specifie a Time To Live for outgoing packets.
		else if (Parameter == L"-i" || Parameter == L"--hop-limits")
		{
			if (Index + 1U < (size_t)argc)
			{
				...
				{
					ConfigurationParameter.IP_HopLimits = (int)UnsignedResult;
				}
				else {
	...
	//Set a long wait periods (in milliseconds) for a response.
		else if (Parameter == L"-w" || Parameter == L"--waiting-time")
		{
			if (Index + 1U < (size_t)argc)
			{
				...
				{
				#if defined(PLATFORM_WIN)
					ConfigurationParameter.SocketTimeout = (int)UnsignedResult;
				#elif (defined(PLATFORM_LINUX) || defined(PLATFORM_MACX))
	...
	//Specifie request server name or port.
		else if (Parameter == L"-p" || Parameter == L"--port")
		{
			...
			//Server name
				UnsignedResult = ServiceNameToPort(Parameter);
				if (UnsignedResult == 0)
				{
					_set_errno(0);
	...
	//Send RAW data with Raw Socket.
		else if (Parameter == L"-raw")
		{
			...
			//Protocol name
				UnsignedResult = ProtocolNameToPort(Parameter);
				if (UnsignedResult == 0)
				{
					_set_errno(0);
		...
		else if (Parameter == L"-socks")
		{
			...
			//IPv6 address
				...
				//Mark port.
					...
					//Server name
						UnsignedResult = ServiceNameToPort(Port);
						if (UnsignedResult == 0)
						{
							_set_errno(0);
